...
export default function CratesAmount(props: {
  ...
  return (
    <View tw="flex-col mt-3">
      ...
      <Text tw="text-base mt-4">
        Total weight of the crates* ({t('Dashboard.ProduceDetails.kilogram')})
      </Text>
      <Controller
        ...
        render={({ field: { onChange, value } }) => (
          <View tw="w-full flex flex-row items-center justify-between">
            <Input
              tw="w-1/2 px-4 bg-white border border-b-0 rounded-sm h-12 mt-1"
              keyboardType="numeric"
              onChangeText={(newVal) => onChangeNumericKeyboard(newVal, onChange, 'generalCrateWeight')}
              value={value?.toString() ?? ''}
            />
            <IconButton
              mode="contained-tonal"
              ...
              onPress={(evt) => {
                evt.stopPropagation();
                onChangeNumericKeyboard(!value || Number(value) <= 0 ? 0 : Number(value) - 1, onChange, 'generalCrateWeight');
              }}
              testID="generalCrateWeight-minus"
            />
            <IconButton
              mode="contained-tonal"
              icon="plus"
              ...
              testID="generalCrateWeight-plus"
            />
          </View>
        )}
      />
