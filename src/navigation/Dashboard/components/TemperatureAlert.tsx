...
export default function TemperatureAlert() {
  ...
  useAppEventListener<[TemperatureAlertEvtDatum]>(
    'DISPATCH_CHECK_IN_TEMPERATURE_ALERT',
    async ({ coolingUnitId, companyId }) => {
      try {
  ...
  return (
    <Portal>
      <Modal visible={isVisible} onDismiss={dismissHandler}>
        <View tw="w-full items-center bg-white rounded-3xl w-11/12 max-w-11/12 h-auto py-4 px-5 self-center space-y-2">
            ...
            {coolingUnitHasSensorIntegration ? (
              ...
              <React.Fragment>
                <View tw="items-start w-full mt-3">
                  <Text variant="TitleSmall">{t('Dashboard.TemperatureAlert.edit')}</Text>
                  <Controller
                    name="temperature"
